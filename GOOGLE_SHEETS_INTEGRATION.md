...
## 🔑 Key Fix: IP / Location Race Condition (Feb 2026)

**Problem:** IP Address, Location, and Organization columns were blank in many rows because page-view events fired before the `ipapi.co` fetch completed.

**Solution implemented in `useAnalytics.ts`:**
1. **Pending Queue** — Events sent before IP is ready are queued in memory
2. **IP Flush** — Once IP resolves, the entire queue is sent with IP data injected
3. **3-Provider Fallback** — `ipapi.co` → `ipwho.is` → `freeipapi.com` (if one fails, tries next)
4. **30-min Cache** — Uses `localStorage` cache on repeat visits for instant IP data
5. **`ipInfoRef`** — A `useRef` ensures all closures always see the latest IP, not stale state

---

## 🚀 Advanced Tracking Engine (60+ Metrics)

### 1. UserBehaviorLibrary (The Master Profile) - 60+ Columns

Add these headers to the **UserBehaviorLibrary** tab:

`Session ID`, `Visitor ID`, `Organization`, `IP Address`, `IP Location`, `Variant`, `Viewport`, `OS`, `Screen Resolution`, `Connection Type`, `Language`, `Dark Theme`, `Time Zone`, `Page Depth`, `Active Tab`, `Session Age`, `Initial Source`, `Landing Page`, `UTM Medium`, `UTM Campaign`, `Referrer Host`, `Search Engine`, `First Visit Date`, `Returning User`, `Cursor Velocity`, `Rage Clicks`, `Scroll Velocity`, `Max Scroll Depth`, `Total Click Count`, `Average Dwell Time`, `Total Active Time`, `Idle Time`, `Tab Switches`, `Exit Intent Triggered`, `Interaction Frequency`, `Last Active Timestamp`, `Lead Generated`, `Form Started`, `Form Abandoned`, `Form Last Field`, `Ebook Downloaded`, `Consultation Requested`, `Partner Inquiry Count`, `Career Inquiry Count`, `WhatsApp Widget Interactions`, `CTA Conversion Rate`, `Interest Banking`, `Interest Retail`, `Interest Logistics`, `Interest School Safety`, `Interest Campus Safety`, `Interest Cash Logistics`, `Interest Command Center`, `Interest Services`, `Interest Commercial`, `Interest Residential`, `Map Interactions`, `Offices Explored`, `Job Views`, `Certification Views`, `Carousel Slide Swipes`, `Testimonials Viewed`, `Philosophy Views`, `Trust Metric Engagement`, `Benefits Section Views`, `Footer Navigation Usage`, `Weighted Rank`, `Engagement Score`, `Segment`, `Hot Lead Flag`, `Intent Rank`, `Dwell Time`, `Interactions`, `Tree`, `Timestamp`

---

## 🛠 Required Setup (Other Tabs)

- **TrafficAnalytics**: `Session ID`, `Visitor ID`, `Page Path`, `Page Title`, `Referrer`, `Traffic Source`, `Organization`, `IP Location`, `IP Address`, `Variant`, `Timestamp`
- **EngagementMetrics**: `Session ID`, `Visitor ID`, `Page URL`, `Duration (sec)`, `Scroll Depth (%)`, `Click Count`, `Engagement Score`, `CTA Clicked`, `Returning User`, `Is Hot Lead`, `Variant`, `Timestamp`
- **BehaviorMetrics**: `Session ID`, `Visitor ID`, `Page URL`, `Category`, `Metric Name`, `Value`, `Element Info`, `Metadata`, `IP Location`, `IP Address`, `Variant`, `Timestamp`
- **Lead Forms** (Contact, Partner, Career, Ebook, Consultation): `[Form Fields]`, `Status`, `IP Location`, `IP Address`, `Variant`, `Timestamp`
...
function doPost(e) {
  ...
  try {
    ...
    var IS_LEAD_FORM = ["ContactForm", "PartnerApps", "CareerApplications", "EbookDownloads", "ConsultationReqs"].indexOf(sheetName) !== -1;
    if (IS_LEAD_FORM) {
...
## 📈 Analytics Strategy & Segmentation

**User behavior segmentation** is the primary focus. The team will analyze the captured user behavior data across the 60+ metrics to create highly targeted segments.

### Sheets Structure (MySQL-equivalent Tables)

| Sheet | Purpose | Key Columns |
|---|---|---|
| `TrafficAnalytics` | Raw page views | IP Address, Page Path, Timestamp |
| `EngagementMetrics` | Time-on-page data | Duration, Scroll Depth, Engagement Score |
| `BehaviorMetrics` | Click/form events | Category, Metric Name, Element |
| `UserBehaviorLibrary` | Master 60-col profile | All metrics, Segment, Hot Lead |
| `DailyReports` | Auto-generated daily KPIs | Cron output |
| `WeeklyReports` | Auto-generated weekly KPIs | Cron output |
| `Watchlist` | IPs to monitor | IP Address column |
| `ChatbotLeads` | Bot interactions | Always has IP now |

### QUERY Formulas for Analytics Questions

**Top 5 pages (excl. homepage):**
```
=QUERY(TrafficAnalytics!A:K,"SELECT C, COUNT(C) WHERE C != '/' GROUP BY C ORDER BY COUNT(C) DESC LIMIT 5 LABEL COUNT(C) 'Visits'",1)
```

**Top 5 IPs (repeat visitors):**
```
=QUERY(TrafficAnalytics!A:K,"SELECT I, COUNT(I) GROUP BY I ORDER BY COUNT(I) DESC LIMIT 5 LABEL COUNT(I) 'Visits'",1)
```

**Avg time spent per page:**
```
=QUERY(EngagementMetrics!A:N,"SELECT B, AVG(D) WHERE B != '' GROUP BY B ORDER BY AVG(D) DESC LIMIT 5 LABEL AVG(D) 'Avg Seconds'",1)
```

---

## 📍 Final Connection
Paste your Web App URL in `src/hooks/useAnalytics.ts` at **Line 12** (constant `GOOGLE_SHEETS_WEB_APP_URL`).

**After editing Apps Script:** Always click **Deploy > Manage Deployments > Create new deployment** and copy the new URL.
