...
import {
    ...
    allTransitSections,
    finalCTA
} from "@/data/mass-transportation-data";
...
import transport1 from "../assets/command_center_1.jpg";
import transport2 from "../assets/command_center_3.jpg";
import transport3 from "../assets/hero-soc.jpg";
import transport4 from "../assets/verticals-surveillance.jpg";
import transport5 from "../assets/hero-drone.jpg";

const heroSlides = [
    { image: transport1, ...transitHeroSlides[0] },
    { image: transport2, ...transitHeroSlides[1] },
    { image: transport3, ...transitHeroSlides[2] },
    { image: transport4, ...transitHeroSlides[3] }
];

import { useContentProtection } from "@/hooks/useContentProtection";

const MassTransportationPage = () => {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col overflow-x-hidden">
            ...
            <main className="flex-grow overflow-x-hidden">
                        ...
                        <div className="container mx-auto px-4 lg:px-8 relative z-10">
                            <div className="max-w-4xl">
                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-card/80 backdrop-blur-sm border border-border rounded-full mb-8 animate-fade-in">
                                    <span className="text-sm font-medium text-primary">{transitHero.badge}</span>
                                </div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 mb-12">
                                    <Link to="/contact"><Button size="lg" className="gap-2 text-base px-8 py-6 shadow-lg shadow-primary/20">Secure Transit Networks<ArrowRight className="w-5 h-5" /></Button></Link>
                                    <Link to="#transit-intelligence"><Button variant="outline" size="lg" className="gap-2 text-base px-8 py-6 bg-card/60 backdrop-blur-md border-white/20 hover:bg-primary hover:text-primary-foreground transition-all duration-300">Explore Transport Intelligence</Button></Link>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-8 max-w-2xl text-xs sm:text-sm text-muted-foreground/80 font-medium">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span>AI-Powered Transit Surveillance</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[10px]">✔</div>
                                        <span>Passenger Safety Monitoring</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[10px]">✔</div>
                                        <span>Intelligent Mobility Analytics</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[10px]">✔</div>
                                        <span>Unified Command & Control Centers</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {/* All Sections */}
                {allTransitSections.map((section, index) => (
...
export default MassTransportationPage;
